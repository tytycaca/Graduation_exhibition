...
bool BitmapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*)L"../GraduationProject v3.0/bitmap.vs", (WCHAR*)L"../GraduationProject v3.0/bitmap.ps");
	if(!result)
	{
		return false;
	}
...
bool BitmapShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
...
bool BitmapShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
											 D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	HRESULT result;
