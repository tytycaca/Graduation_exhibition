...
bool TextClass::SetWoodStackCnt(int woodStackCnt, ID3D11DeviceContext* deviceContext)
{
	char tempString[1][32];
	char LootModelCountString[32];
	bool result;

	_itoa_s(woodStackCnt, tempString[0], 10);

	strcpy_s(LootModelCountString, tempString[0]);

	result = UpdateSentence(m_sentence[11], LootModelCountString, 690, 900, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result) return false;
...
bool TextClass::SetAmethCnt(int amethCnt, ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateSentence(m_sentence[9], LootModelCountString, 760, 900, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result) return false;
