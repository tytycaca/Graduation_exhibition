...
bool ShaderManagerClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	// Create the pointlight shader object.
	m_PonitLightShader = new PointLightShaderClass;
	if (!m_LightShader)
	{
		return false;
	}

	// Initialize the pointlight shader object.
	result = m_PonitLightShader->Initialize(device, hwnd);
	if (!result)
	{
		MessageBox(hwnd, L"Could not initialize the light shader object.", L"Error", MB_OK);
		return false;
...
void ShaderManagerClass::Shutdown()
{
	...
	// Release the pointlight shader object.
	if (m_PonitLightShader)
	{
		m_PonitLightShader->Shutdown();
		delete m_PonitLightShader;
		m_PonitLightShader = 0;
	}
...
bool ShaderManagerClass::RenderFireTextureShader(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
	...
	D3DXVECTOR2 distortion3, float distortionScale, float distortionBias)
{
	...
	result = m_FireShader->Render(deviceContext, indexCount, worldMatrix, viewMatrix, projectionMatrix, fireTexture, noiseTexture, alphaTexture, frameTime,
...
bool ShaderManagerClass::RenderPointLightShader(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
	...
	D3DXVECTOR4 lightPosition[])
{
	...
	// Render the model using the pointlight shader.
	result = m_PonitLightShader->Render(deviceContext, indexCount, worldMatrix, viewMatrix, projectionMatrix, texture, diffuseColor, lightPosition);
	if (!result)
	{
		return false;
	}
